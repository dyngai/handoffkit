...
//
// For concurrent Merges to be conflict-free it must behave as an ASSOCIATIVE,
// COMMUTATIVE, and IDEMPOTENT join over logical values: applying deltas in any
// grouping or arrival order, and replaying a delta, must converge to the same
// result. This is the contract that lets the Corpus be shared state without a
// per-agent lock dance (sketch/handoffkit.go: Corpus).
...
// (NOT associative, commutative, and idempotent), so it is correct only for a
// key with a single writer (for example a per-handoff artifact key minted by
// one agent). Do not use it for a key multiple agents Merge concurrently; use a
// policy such as UnionStrings there.
...
// string or a []string on either side. It is associative, commutative, and
// idempotent, so concurrent Merges of the same key are conflict-free regardless
// of order, the canonical "shared knowledge, reconciled without locks" case.
...
// writes are reconciled by a MergeFunc. It is the repo's one deliberate use of
// shared state, the blackboard the design keeps for knowledge (knowledge stays
// put and is referenced, not channelled as prose). It is intentionally local and
// small: enough to make MemoryRef resolve and to make merge policy explicit and
// testable.
...
// MergeFunc being associative, commutative, and idempotent, not from the lock
// (the lock only guards the map).
